...
class MyFrame(wx.Frame):
    ...
    def ProcessFrame(self, frame):
        ...
        draw = cv3.cvtColor(segment, cv3.COLOR_GRAY2RGB)

        # draw some helpers for correctly placing hand
        cv3.circle(draw,(self.imgWidth/2,self.imgHeight/2),3,[255,102,0],2)       
        cv3.rectangle(draw, (self.imgWidth/3,self.imgHeight/3), (self.imgWidth*2/3, self.imgHeight*2/3), [255,102,0],2)
        ...
        cv3.putText(draw, str(nofingers), (30,30), cv3.FONT_HERSHEY_SIMPLEX, 1, (255,255,255))
        return draw
    ...
    def SegmentArm(self, frame):
        ...
        frame = cv3.morphologyEx(frame, cv3.MORPH_CLOSE, kernel)
        ...
        flood = frame.copy()
        cv3.floodFill(flood, mask, (self.imgWidth/2,self.imgHeight/2), 255, flags=4|(255<<8))

        ret,flooded = cv3.threshold(flood, 129, 255, cv3.THRESH_BINARY)

        return flooded


    def FindHullDefects(self, segment):
        _,contours,hierarchy = cv3.findContours(segment, cv3.RETR_TREE, cv3.CHAIN_APPROX_SIMPLE)

        # find largest area contour
        max_area = -1
        for i in range(len(contours)):
            area = cv3.contourArea(contours[i])
            if area>max_area:
                cnt = contours[i]
                max_area = area

        cnt = cv3.approxPolyDP(cnt,0.01*cv3.arcLength(cnt,True),True)
        hull = cv3.convexHull(cnt, returnPoints=False)
        defects = cv3.convexityDefects(cnt, hull)
    ...
    def DetectNumberFingers(self, contours, defects, draw):
        ...
        for i in range(defects.shape[0]):
            ...
            # draw the hull
            cv3.line(draw,start,end,[0,255,0],2)
            ...
            if angle(np.subtract(start,far), np.subtract(end,far)) < angleFingerDeg/180.0*np.pi:
                ...
                # draw point as green
                cv3.circle(draw,far,5,[0,255,0],-1)
            else:
                # draw point as red
                cv3.circle(draw,far,5,[255,0,0],-1)
...
def main():
    device = cv3.CAP_OPENNI
    capture = cv3.VideoCapture(device)
